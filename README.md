...
- Attaches to T-slot framing, using M4 screws.

 
## Design Documents

[design.md] has detailed interface specifications.
...
[tslot_nut.py] generates a parametric T-slot nut with a hex nut pocket and spring arm to hold its position.
...
[connector_plate.py] generates the connector plates to fit into the cutouts.
...
[din35_mount_stabilizer.py] generates a prismatic standoff that sits below the DIN rail to give it stability.
...
If you are machining the box yourself, see [manufacturing-notes.md].

See [design.md] for the electronics inside the box.
...
[idec_fb_m12_adapter.FCStd] and [idec_fb_m12_washer.FCStd] "sandwich" the M12 connector to adapt it to the larger punchout.
idec_fb_m12_adapter.FCStd must be printed with supports, preferably two-material breakaway supports.
Because these add thickness, the O-ring on the M12 must be removed.
Additionally, these have an anti-rotation feature, and a 1mm wide x 2mm long (radially outward) slot needs to be dremeled into the top of the punchout.
The anti-rotation feature is triangular, the slot does not have to be completely through.

[generic_adapter_plate.py] generates adapter plates that adapt the back of these boxes to attach to the T-slot framing.
